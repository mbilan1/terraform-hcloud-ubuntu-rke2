...
output "_test_counts" {
  ...
  value = {
    ...
    cert_manager_namespace        = length(kubernetes_namespace_v1.certificate_manager)
    cert_manager_release          = length(helm_release.certificate_manager)
    hccm_secret                   = length(kubernetes_secret_v1.cloud_controller_token)
    hccm_release                  = length(helm_release.cloud_controller)
    csi_release                   = length(helm_release.hcloud_csi)
    kured_namespace               = length(kubernetes_namespace_v1.reboot_daemon)
    kured_release                 = length(helm_release.reboot_daemon)
